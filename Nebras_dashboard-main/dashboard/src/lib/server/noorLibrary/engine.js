...
async function processBook({ url, bookId, sections }) {
	...
	if (decision.kind === 'create_main') {
		// أعلى مستوى من الإنشاء — main + sub + secondary لكي يبقى المحتوى
		// دائماً تحت الهيكل الذهبي الكامل.
		...
		const createdSecondary = await createSecondarySectionAdmin(
			subId,
			decision.newSecondaryName || decision.newSubName || createdSub.name
		);
		...
		const createdSecondary = await createSecondarySectionAdmin(
			subId,
			decision.newSecondaryName || decision.newSubName || created.name
		);
		secondaryId = String(createdSecondary.id);
		if (!createdSecondary.alreadyExisted) {
			createdSectionsIds.push(secondaryId);
