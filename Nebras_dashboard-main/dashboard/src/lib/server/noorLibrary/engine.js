...
async function processBook({ url, bookId, sections }) {
	...
	const fallbackSecondaryName = () => {
		const hints = Array.isArray(meta.categoryHints) ? [...meta.categoryHints].reverse() : [];
		const fromHint = hints.map((h) => String(h || '').trim()).find(Boolean);
		return String(decision.newSecondaryName || fromHint || meta.title || 'كتب عامة').trim().slice(0, 80) || 'كتب عامة';
	};

	const ensureSecondarySection = async (targetSubId, requestedName, parentSubName = '') => {
		const name = String(requestedName || fallbackSecondaryName()).trim().slice(0, 80) || 'كتب عامة';
	...
	if (decision.kind === 'create_main') {
		...
		await ensureSecondarySection(subId, decision.newSecondaryName, createdSub.name);
		...
		await ensureSecondarySection(subId, decision.newSecondaryName, created.name);
	} else if (decision.kind === 'create_secondary') {
		subId = decision.subId;
		await ensureSecondarySection(subId, decision.newSecondaryName);
	}
