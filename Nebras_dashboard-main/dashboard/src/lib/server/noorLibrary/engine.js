...
async function processBook({ url, bookId, sections }) {
	...
	const rememberCreatedSection = ({ level, id, name, parentId = '', parentName = '' }) => {
	...
	if (decision.kind === 'create_main') {
		// أعلى مستوى من الإنشاء — main + sub + secondary في مسار واحد.
		...
		const createdSecondary = await createSecondarySectionAdmin(
			subId,
			decision.newSecondaryName || decision.newSubName || 'كتب عامة'
		);
	...
	if (!secondaryId) {
		throw Object.assign(new Error('فشل تحديد secondaryId بعد التصنيف — Noor يتطلب مساراً ثلاثياً كاملاً.'), {
