...
async function processBook({ url, bookId, sections }) {
	...
	if (decision.kind === 'create_main') {
		...
		if (decision.newSecondaryName) {
			...
			if (!createdSec.alreadyExisted) {
				...
				createdSections.push({
					...
					path: `${createdMain.name} > ${createdSub.name} > ${createdSec.name}`
				});
				sectionsCreatedDelta += 1;
				await bumpStats({ sectionsCreatedDelta: 1 }).catch(() => {});
				await notifyFcmSectionCreated({
		...
		if (!created.alreadyExisted) {
			...
			createdSections.push({
				...
				path: `${parentMain?.name || ''} > ${created.name}`
			});
			sectionsCreatedDelta += 1;
		...
		if (decision.newSecondaryName) {
			...
			if (!createdSec.alreadyExisted) {
				...
				createdSections.push({
					...
					path: `${parentMain?.name || ''} > ${created.name} > ${createdSec.name}`
				});
				sectionsCreatedDelta += 1;
				await bumpStats({ sectionsCreatedDelta: 1 }).catch(() => {});
				await notifyFcmSectionCreated({
		...
		if (!created.alreadyExisted) {
			...
			createdSections.push({
				...
				path: `${parentMain?.name || ''} > ${parentSub?.name || ''} > ${created.name}`
			});
			sectionsCreatedDelta += 1;
			await bumpStats({ sectionsCreatedDelta: 1 }).catch(() => {});
			await notifyFcmSectionCreated({
	...
	if (!secondaryId) {
		const fallbackName = decision.newSecondaryName || 'متفرقات';
		const created = await createSecondarySectionAdmin(subId, fallbackName);
		secondaryId = String(created.id);
		if (!created.alreadyExisted) {
			createdSectionsIds.push(secondaryId);
			const parentSub = sections.index.subsById[subId];
			const parentMain = parentSub ? sections.index.mainsById[String(parentSub.main_section ?? mainId)] : null;
			createdSections.push({
				level: 'secondary',
				id: secondaryId,
				name: created.name,
			...
			await appendLog({
				level: 'success',
				message: `قسم ثانوي جديد أُنشئ آلياً لإكمال الهيكل الثلاثي: "${created.name}"`,
				sectionId: created.id,
				parentId: subId,
				kind: 'secondary_section_created'
			}).catch(() => {});
	...
	if (!secondary) {
		throw Object.assign(new Error('فشل تحديد قسم ثانوي — محرّك نور يتطلّب المسار الثلاثي الكامل.'), {
			reason: 'secondary_required_after_classify',
			status: 500
		});
	}
