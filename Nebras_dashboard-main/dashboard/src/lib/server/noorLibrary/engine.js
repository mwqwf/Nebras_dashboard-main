...
async function processBook({ url, bookId, sections }) {
	...
	if (decision.kind === 'create_main') {
		...
		if (!created.alreadyExisted) {
			...
			const parentMain = parentSub
				? sections.index.mainsById[String(parentSub.main_section ?? '')]
				: null;
			createdSections.push({
				level: 'secondary',
				id: secondaryId,
				...
				path: `${parentMain?.name || ''} > ${parentSub?.name || subId} > ${created.name}`.replace(/^ > /, '')
			});
			sectionsCreatedDelta += 1;
			await bumpStats({ sectionsCreatedDelta: 1 }).catch(() => {});
			await notifyFcmSectionCreated({
	...
	if (!secondaryId) {
		throw Object.assign(new Error('فشل تحديد secondaryId بعد التصنيف — الهيكل الثلاثي مطلوب لمكتبة نور.'), {
	...
	if (!secondary) {
		throw Object.assign(new Error('secondary لم يُعثر عليه بعد التصنيف — لا يُقبل محتوى Noor خارج الهيكل الثلاثي.'), {
