...
function fallbackSecondarySectionName(meta) {
	...
	return hint.slice(0, 80) || 'كتب عامة';
}
...
async function processBook({ url, bookId, sections }) {
	...
	if (decision.kind === 'create_main') {
		...
		if (!created.alreadyExisted) {
			...
			createdSections.push({
				...
				name: created.name,
				parentId: mainId,
				path: `${parentMain?.name || mainId} > ${created.name}`
			});
			sectionsCreatedDelta += 1;
		...
		if (decision.newSecondaryName) {
			...
			if (!createdSecondary.alreadyExisted) {
				...
				createdSections.push({
					...
					name: createdSecondary.name,
					parentId: subId,
					path: `${parentMain?.name || mainId} > ${created.name} > ${createdSecondary.name}`
				});
				sectionsCreatedDelta += 1;
				await bumpStats({ sectionsCreatedDelta: 1 }).catch(() => {});
				await notifyFcmSectionCreated({
		...
		if (!created.alreadyExisted) {
			...
			createdSections.push({
				...
				name: created.name,
				parentId: subId,
				path: `${parentMain?.name || ''} > ${parentSub?.name || subId} > ${created.name}`
			});
			sectionsCreatedDelta += 1;
			await bumpStats({ sectionsCreatedDelta: 1 }).catch(() => {});
			await notifyFcmSectionCreated({
	...
	if (!secondaryId) {
		const secondaryName = decision.newSecondaryName || fallbackSecondarySectionName(meta);
		...
		if (!created.alreadyExisted) {
			...
			createdSections.push({
				...
				name: created.name,
				parentId: subId,
				path: `${parentMain?.name || mainId} > ${parentSub?.name || subId} > ${created.name}`
			});
			sectionsCreatedDelta += 1;
			await bumpStats({ sectionsCreatedDelta: 1 }).catch(() => {});
			await notifyFcmSectionCreated({
			...
			await appendLog({
				level: 'success',
				message: `قسم ثانوي جديد أُنشئ آلياً: "${created.name}" تحت "${parentSub?.name || subId}"`,
				sectionId: created.id,
				parentId: subId,
				kind: 'secondary_section_created'
			}).catch(() => {});
