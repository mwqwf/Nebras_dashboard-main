		...
		if (decision.newSecondaryName) {
			...
			if (!createdSec.alreadyExisted) {
				...
				createdSections.push({
					...
					path: [parentMain?.name || '', created.name, createdSec.name].filter(Boolean)
				});
				sectionsCreatedDelta += 1;
				await bumpStats({ sectionsCreatedDelta: 1 }).catch(() => {});
				await notifyFcmSectionCreated({
...
export async function runEngineTick() {
	...
	for (const link of discovery.newBooks) {
		...
		try {
			...
			createdSectionsReport.push(...(r.createdSections || []));
			sample.push({
				fileId: r.fileId,
				title: r.title,
				url: link.url,
	...
	return {
		...
		sample,
		report: {
			booksAdded: processed,
			sectionsCreated: createdSectionsTotal,
			createdSections: createdSectionsReport
		}
	};
}
