...
async function processBook({ url, bookId, sections }) {
	...
	async function createSecondaryForSub(targetSubId, sectionName, parentSubName = '') {
		const created = await createSecondarySectionAdmin(targetSubId, sectionName || 'موضوعات عامة');
		const createdId = String(created.id);
		if (!created.alreadyExisted) {
			createdSectionsIds.push(createdId);
	...
	if (decision.kind === 'create_main') {
		...
		secondaryId = await createSecondaryForSub(subId, decision.newSecondaryName);
	}
	...
	if (!subId) {
		...
		secondaryId = await createSecondaryForSub(subId, 'كتب عامة', createdSub.name);
	}

	if (!secondaryId && subId) {
		secondaryId = await createSecondaryForSub(
			subId,
			decision.newSecondaryName || 'موضوعات عامة'
		);
	}
