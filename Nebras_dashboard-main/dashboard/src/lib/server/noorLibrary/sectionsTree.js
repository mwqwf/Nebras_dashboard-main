...
const NORMALIZED_TYPO_PATTERNS = TYPO_SECTION_PATTERNS.map(normalizeArabic).filter(Boolean);
...
export function isBlacklistedSectionName(name) {
	...
	return NORMALIZED_TYPO_PATTERNS.some((pattern) => n.includes(pattern));
}
