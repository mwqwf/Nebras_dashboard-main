...
/**
 * مقاطع typo معروفة لا يجب أن تظهر للمصنّف حتى لو تغيّر باقي اسم القسم.
 * وجود المقطع داخل الاسم كافٍ لحذفه مع كل أبنائه من شجرة Noor.
 */
const TYPO_SECTION_NAME_FRAGMENTS = Object.freeze([
	'بتدكصهك',
	'بترخيصه'
]);
...
export function isBlacklistedSectionName(name) {
	...
	if (!n) return false;
	return NORMALIZED_BLACKLIST.has(n) || NORMALIZED_TYPO_FRAGMENTS.some((fragment) => n.includes(fragment));
}
