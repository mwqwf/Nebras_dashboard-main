...
const BLACKLISTED_SECTION_CONTAINS = Object.freeze([
	'بتدكصهك',
	'دروس بترخيص'
]);
...
const NORMALIZED_BLACKLIST_CONTAINS = BLACKLISTED_SECTION_CONTAINS
	.map(normalizeArabic)
	.filter(Boolean);
...
export function isBlacklistedSectionName(name) {
	...
	return NORMALIZED_BLACKLIST_CONTAINS.some((blocked) => n.includes(blocked));
}
