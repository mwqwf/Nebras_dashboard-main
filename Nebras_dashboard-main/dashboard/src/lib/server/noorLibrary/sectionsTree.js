...
const BLACKLISTED_TYPO_FRAGMENTS = Object.freeze([
	'بتدكصهك',
	'بترخيصها',
	'بترخيصه'
]);
...
const NORMALIZED_TYPO_FRAGMENTS = BLACKLISTED_TYPO_FRAGMENTS.map(normalizeArabic).filter(Boolean);
...
export function isBlacklistedSectionName(name) {
	...
	return NORMALIZED_TYPO_FRAGMENTS.some((fragment) => n.includes(fragment));
}
