 ...
 *       NOOR_REQUIRE_STEALTH=true|false — منع الرجوع إلى Puppeteer العاديّ
 *                                          بلا stealth (افتراضي true).
...
/**
 * يحاول تحميل puppeteer-extra + stealth plugin. في التشغيل الافتراضي لا نرجع
 * إلى Puppeteer العاديّ، لأنّ مكتبة نور خلف Cloudflare وطلب المستخدم صريح:
 * كلّ تشغيل للمحرّك يجب أن يكون عبر Stealth Mode.
 */
async function loadPuppeteer() {
	...
	try {
		...
		if (requireStealth) {
			...
			state.lastError =
				'Puppeteer Stealth مطلوب لمحرّك مكتبة نور لكن puppeteer-extra أو stealth plugin غير متاح.';
			return null;
		}

		// retry with plain puppeteer عند تعطيل NOOR_REQUIRE_STEALTH صراحةً فقط.
		try {
...
export async function isPuppeteerEnabled() {
	// الافتراضي true لأنّ مسار مكتبة نور المطلوب هنا هو Stealth Mode. يمكن
	// تعطيله صراحةً في بيئات serverless التي تعتمد crawl4ai فقط.
