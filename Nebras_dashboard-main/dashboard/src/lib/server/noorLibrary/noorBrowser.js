 ...
 *       NOOR_REQUIRE_STEALTH=true|false — رفض التشغيل بدون stealth (افتراضي true).
 ...
 * ممنوع افتراضياً لأنّه يخلّ بشرط التشغيل الخفي لمحرك مكتبة نور.
 */
async function loadPuppeteer() {
	...
	try {
		...
		if (requireStealth) {
			...
			state.lastError =
				'Stealth Mode مطلوب لمحرك مكتبة نور، لكن puppeteer-extra أو stealth-plugin غير متاح.';
			return null;
		}
		// retry with plain puppeteer (بدون stealth — لن يجتاز Cloudflare غالباً
		// ولا يُسمح به إلا عند ضبط NOOR_REQUIRE_STEALTH=false صراحةً).
		try {
...
export async function isPuppeteerEnabled() {
	// الافتراضي true لأن محرك Noor يعتمد متصفحاً حقيقياً مع Stealth Mode.
	// عطّله فقط في بيئات لا تملك Chromium أو عندما يعتمد التشغيل على crawl4ai.
